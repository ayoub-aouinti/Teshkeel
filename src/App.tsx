...
import type { AppState, DocumentData, Operation } from './types'
...
export default function App() {
  ...
  const handleTextSubmit = (text: string, fileName: string) => { void process(text, fileName) }
  ...
  const handleOpenOperation = (op: Operation) => {
    setDoc({ id: op.id, originalText: op.original_text, tashkeelText: op.tashkeel_text, fileName: op.title })
    setState('editing')
  }

  const handleReset = () => { setState('idle'); setDoc(null); setError('') }

  return (
    <div className="min-h-screen bg-[#faf9f6] flex flex-col">
      <Header
        onDashboard={() => setState('dashboard')}
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 pb-16">
        ...
        {state === 'editing' && doc !== null && (
          <EditorPanel
            ...
            onDocSaved={(id) => setDoc({ ...doc, id })}
          />
        )}
        {state === 'dashboard' && (
          <Dashboard onOpen={handleOpenOperation} onBack={handleReset} />
        )}
        {state === 'error' && (
